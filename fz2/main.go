...
func run(args []string) int {
	if len(args) == 0 {
		fmt.Fprintln(os.Stderr, "usage: fz2 <path> <query> | fz2 -r <path> <query>")
		return 1
	}

	if args[0] == "-r" {
		if len(args) != 3 {
			fmt.Fprintln(os.Stderr, "usage: fz2 -r <path> <query>")
			return 1
		}
	...
	if len(args) != 2 {
		fmt.Fprintln(os.Stderr, "usage: fz2 <path> <query>")
		return 1
	}

	fs(args[0], args[1], true)
	return 0
}
